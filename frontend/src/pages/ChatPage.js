...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    axios.get(`/api/messages/${active._id}`)
  ...
  // Send GIF or Sticker URL as an image message
      ...
      {showGroup && (
        <div className="modal-overlay"
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
                ...
                {picked.map(u => (
                  <span key={u._id} className="tag">
                    ...
                    <span className="remove"
                      onClick={() => setPicked(prev => prev.filter(x => x._id !== u._id))}>
                      ×
                    </span>
                  </span>
                ))}
            ...
            {searchRes
              .filter(u => !picked.find(p => p._id === u._id))
              .map(u => (
